	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			axios.get('api/delete-admissionType/' + id)
				.then(() => {
					admissiontypes.value = admissiontypes.value.filter(role => {
						return role.id != id;
					})
					...
					<div class="row">
						<div class="col-sm-12">
							<table id="example2" class="table table-striped  dataTable text-primary">
								...
								<template v-else>
									<tbody v-if="admissiontypes.length!=0">
										<tr role="row" v-for="(admission, index) in admissiontypes" :key="admission.index">
											<td>{{ index + 1 }}</td>
