...
watch(perPage,(newVal)=>{
	    loadData(
              admissiontypes.value.path + "?page=" + admissiontypes.value.current_page,
                newVal
            );
})
...
const filterByStatus = computed(()=>{
	if (search_key.value != "") {
                ...
                return admissiontypes.value.data.filter(
                ...
                return admissiontypes.value.data;
            }
})
function deleteData(id) {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			axios.get('api/delete-admissionType/' + id)
				.then(() => {
					admissiontypes.value.data = admissiontypes.value.data.filter(role => {
						return role.id != id;
					})
...
function addData() {
	...
	isSubmit.value = true
	axios.defaults.headers.common['Authorization'] = token.value
	axios.post('api/add-admissionType', {
		'name': name.value,
		}).then(() => {
			isHide.value = false
			loadData('/api/get-admissiontypes');
...
function editUser(){
	...
	axios.defaults.headers.common['Authorization'] = token.value
	isSubmit.value = true
	axios.post('api/update-admissionType/'+dataId.value, {
		'name': name.value
	}).then(() => {
			isHide.value = false
			isEdit.value = false
...
function showData(id){
	...
	axios.defaults.headers.common['Authorization'] = token.value
	axios.get('api/get-admissionType/'+id)
	.then(res=>{
		isHide.value = true
		isEdit.value = true
		dataId.value = id
...
<template>
	<div class="page-content">
		...
		<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-2">
			<div class="breadcrumb-title pe-3">Admission Types</div>
			<div class="ps-3">
		...
		<div v-if="isHide" class="card border-top border-0 border-4 border-primary">
			<div class="card-body p-5">
				...
				<div class="card-title d-flex align-items-center">
					...
					<h5 class="mb-0 text-primary">Admission Types Registration</h5>
				</div>
				<hr>
				<form class="row g-3 ">
					<div class="col-md-12">
						<label for="validationCustom01" class="form-label">Admission Type Name</label>
						<input v-model="name" type="text" class="form-control border-primary" id="validationCustom01"
							placeholder="Enter admission type name">
		...
		<div v-if="isHide==false" class="card border-top border-0 border-4 border-primary">
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="example2" class="table table-striped dataTable text-primary">
                                            ...
                                            <template v-else>
                                           <tbody v-if="admissiontypes.data !=  null">
                                            <tr role="row" v-for="(admission,index) in filterByStatus" :key="admission.id">
                                            <th scope="row">{{ admissiontypes.from +index }} </th>
                                            <td>{{ admission.name }}</td>
                                            <td>{{ format(new Date(admission.created_at), 'MMMM do, yyyy') }}</td>
                                        <td>{{ format(new Date(admission.updated_at), 'MMMM do, yyyy') }}</td>
			...
			<li><button @click.prevent="showData(admission.id)" class="btn btn-outline-info dropdown-item "><i
						class="fadeIn animated bx bx-edit"></i>Edit</button>
			</li>
			<li><button @click.prevent="deleteData(admission.id)"
     ...
     <div class="row mb-0">
        <div class="col-sm-12 col-md-8">
            <div class="dataTables_info">
                Showing {{ admissiontypes.from }} to {{ admissiontypes.to }} of {{ admissiontypes.total }} entries
            </div>
        </div>
                <div class="col-sm-12 col-md-4">
                  <div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
                        <ul class="pagination">
                        <li @click="admissiontypes.current_page ==1? '': loadData(admissiontypes.first_page_url)" :class="{disabled:admissiontypes.current_page == 1,'paginate_button page-item next': true,}" id="example2_previous">
                                <a  aria-controls="example2" data-dt-idx="0" tabindex="0"
                                     class="page-link"><i class="fadeIn animated bx bx-chevrons-left"></i></a>
                        </li>
                        <li @click="admissiontypes.prev_page_url !=null? loadData       (admissiontypes.prev_page_url): ''" :class="{disabled:admissiontypes.prev_page_url ==null,'paginate_button page-item previous': true,}" id="simpletable_next">
                               <a  aria-controls="example2" data-dt-idx="1" tabindex="0"
                                    class="page-link"><i class="fadeIn animated bx bx-chevron-left"></i></a>
                         </li>
                        <li @click="loadData(admissiontypes.path +'?page=' +admissiontypes.current_page)" class="paginate_button page-item active">
                               <a aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"> {{ admissiontypes.current_page }}</a>
                         </li>
                        <li v-if="admissiontypes.last_page >admissiontypes.current_page +1"
                        @click="loadData(admissiontypes.path +'?page=' +(admissiontypes.current_page +1))"  class="paginate_button page-item">                    
                                <a  aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"> {{admissiontypes.current_page + 1 }}
                                                    </a>
                         </li>
                        <li v-if="admissiontypes.last_page >admissiontypes.current_page +2" 
                        @click="loadData(admissiontypes.path +'?page=' +(admissiontypes.current_page +2))" class="paginate_button page-item"><a  aria-controls="example2" data-dt-idx="6" tabindex="0"  class="page-link">{{ admissiontypes.current_page + 2 }} </a>
                        </li>
                          <li @click="admissiontypes.next_page_url !=null ? loadData(admissiontypes.next_page_url): ''" :class="{disabled:admissiontypes.next_page_url ==null,
                            'paginate_button page-item next': true,}" id="simpletable_next">
                                <a  aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"><i class="fadeIn animated bx bx-chevron-right"></i>
                                </a>
                        </li>
                        <li @click="admissiontypes.current_page == admissiontypes.last_page? '': loadData(admissiontypes.last_page_url)" :class="{disabled:admissiontypes.current_page == admissiontypes.last_page,'paginate_button page-item next': true,}"  id="simpletable_next">
                            <a  aria-controls="example2" data-dt-idx="7" tabindex="0"
                                class="page-link"><i class="fadeIn animated bx bx-chevrons-right"></i>
                                                    </a>
                         </li>
                        <li @click="loadData(admissiontypes.path +'?page=' +admissiontypes.current_page)"  class="paginate_button page-item" id="simpletable_next">
                            <a  aria-controls="example2" data-dt-idx="7" tabindex="0"
