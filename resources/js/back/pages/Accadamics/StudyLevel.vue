...
watch(perPage,(newVal)=>{
	    loadData(
              studytypes.value.path + "?page=" + studytypes.value.current_page,
                newVal
            );
})
...
const filterByStatus = computed(()=>{
	if (search_key.value != "") {
                ...
                return studytypes.value.data.filter(
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			axios.get('api/delete-studyLevel/' + id)
				.then(() => {
					studytypes.value.data = studytypes.value.data.filter(role => {
						return role.id != id;
					})
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="example2" class="table table-striped dataTable text-primary">
                                            ...
                                            <template v-else>
                                           <tbody v-if="studytypes.data !=  null">
                                            <tr role="row" v-for="(admission,index) in filterByStatus" :key="admission.id">
                                            <th scope="row">{{ studytypes.from +index }} </th>
     ...
     <div class="row mb-0">
        <div class="col-sm-12 col-md-8">
            <div class="dataTables_info">
                Showing {{ studytypes.from }} to {{ studytypes.to }} of {{ studytypes.total }} entries
            </div>
        </div>
                <div class="col-sm-12 col-md-4">
                  <div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
                        <ul class="pagination">
                        <li @click="studytypes.current_page ==1? '': loadData(studytypes.first_page_url)" :class="{disabled:studytypes.current_page == 1,'paginate_button page-item next': true,}" id="example2_previous">
                                <a  aria-controls="example2" data-dt-idx="0" tabindex="0"
                                     class="page-link"><i class="fadeIn animated bx bx-chevrons-left"></i></a>
                        </li>
                        <li @click="studytypes.prev_page_url !=null? loadData       (studytypes.prev_page_url): ''" :class="{disabled:studytypes.prev_page_url ==null,'paginate_button page-item previous': true,}" id="simpletable_next">
                               <a  aria-controls="example2" data-dt-idx="1" tabindex="0"
                                    class="page-link"><i class="fadeIn animated bx bx-chevron-left"></i></a>
                         </li>
                        <li @click="loadData(studytypes.path +'?page=' +studytypes.current_page)" class="paginate_button page-item active">
                               <a aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"> {{ studytypes.current_page }}</a>
                         </li>
                        <li v-if="studytypes.last_page >studytypes.current_page +1"
                        @click="loadData(studytypes.path +'?page=' +(studytypes.current_page +1))"  class="paginate_button page-item">                    
                                <a  aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"> {{studytypes.current_page + 1 }}
                                                    </a>
                         </li>
                        <li v-if="studytypes.last_page >studytypes.current_page +2" 
                        @click="loadData(studytypes.path +'?page=' +(studytypes.current_page +2))" class="paginate_button page-item"><a  aria-controls="example2" data-dt-idx="6" tabindex="0"  class="page-link">{{ studytypes.current_page + 2 }} </a>
                        </li>
                          <li @click="studytypes.next_page_url !=null ? loadData(studytypes.next_page_url): ''" :class="{disabled:studytypes.next_page_url ==null,
                            'paginate_button page-item next': true,}" id="simpletable_next">
                                <a  aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"><i class="fadeIn animated bx bx-chevron-right"></i>
                                </a>
                        </li>
                        <li @click="studytypes.current_page == studytypes.last_page? '': loadData(studytypes.last_page_url)" :class="{disabled:studytypes.current_page == studytypes.last_page,'paginate_button page-item next': true,}"  id="simpletable_next">
                            <a  aria-controls="example2" data-dt-idx="7" tabindex="0"
                                class="page-link"><i class="fadeIn animated bx bx-chevrons-right"></i>
                                                    </a>
                         </li>
                        <li @click="loadData(studytypes.path +'?page=' +studytypes.current_page)"  class="paginate_button page-item" id="simpletable_next">
                            <a  aria-controls="example2" data-dt-idx="7" tabindex="0"
