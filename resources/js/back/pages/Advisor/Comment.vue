...
function addData() {
	...
	axios.post('api/comment-on-document', form-data, {
		headers: {
               'Content-Type': 'multipart/form-data',
             },
			}).then(() => {
			isHide.value = false
			loadData('api/get-single-document-comment/'+documentId);
...
function editData(){
	...
	let formdata = new FormData();
		...
		formdata.append("attached_file", attached_file.value);
		formdata.append("document_id", userId.value);
	axios.post('api/update-comment/'+comment_id.value, formdata, {
		  headers: {
               'Content-Type': 'multipart/form-data',
             },
			}).then(() => {
	...
	<div class="page-content">
			...
			<div class="card-body p-5">
				...
				<form class="row g-3 " enctype="multipart/form-data">
					...
					<div class="col-md-6" v-if="isUser==false">
					 ...
					 <div class="col-12" v-if="isUser==false">
		...
		<div v-if="isHide==false" class="card border-top border-0 border-4 border-primary">
			...
			<div class="table-responsive">
				<RouterLink :to="{name:'ViewDocument'}"  style="float: right;" class="btn btn-outline-warning "
								type="button">
												...
												<div class="btn-group" role="group"
													...
													<div class="btn-group" role="group">
														...
														<ul class="dropdown-menu" aria-labelledby="btnGroupDrop1" style="">
															<li v-if="comment.user_id == comment.document.advisor_id"><button @click.prevent="showData(comment.id)" class="btn btn-outline-info dropdown-item "><i
																		class="fadeIn animated bx bx-edit"></i>Edit</button>
															</li>
															<li v-else ><button @click.prevent="showData(comment.id); isUser = true" class="btn btn-outline-info dropdown-item "><i class="lni lni-eye"></i>View</button>
															</li>
															
															
															<li v-if="comment.user_id == comment.document.advisor_id" ><button  @click.prevent="deleteData(comment.id)"
