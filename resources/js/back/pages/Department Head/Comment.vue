...
function addData() {
	...
	axios.post('api/comment-on-document', formdata, {
		headers: {
			...
			isHide.value = false
			loadData('api/get-single-document-comment-deptHead/'+documentId);
...
function editData(){
	...
	let formdata = new FormData();
		...
		formdata.append("attached_file", attached_file.value);
		formdata.append("document_id", document_id.value);
	axios.post('api/update-comment-deptHead/'+comment_id.value, formdata, {
		headers: {
               'Content-Type': 'multipart/form-data',
             },
			}).then(() => {
	...
	<div class="page-content">
		...
		<div v-if="isHide==false" class="card border-top border-0 border-4 border-primary">
			...
			<div class="table-responsive">
				<RouterLink :to="{name:'ViewDocumentDeptHead'}" style="float: right;" class="btn btn-outline-warning "
									type="button">
												...
												<div class="btn-group" role="group"
													...
													<div class="btn-group" role="group">
														...
														<ul class="dropdown-menu" aria-labelledby="btnGroupDrop1" style="">
															<li ><button @click.prevent="showData(comment.id)" class="btn btn-outline-info dropdown-item "><i
																		class="fadeIn animated bx bx-edit"></i>Edit</button>
															</li>
															
															
															
															<li><button  @click.prevent="deleteData(comment.id)"
