...
function loadData() {
	...
	axios.get('api/get-student-document-deptHead')
		.then(res => {
...
function approveDocument(id) {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			axios.defaults.headers.common['Authorization'] = token.value;
			axios.get('api/approve-document-deptHead/' + id)
				.then(() => {
					loadData();
...
function addData() {
	...
	axios.post('api/comment-on-document-deptHead', formdata, {
		headers: {
								...
								<template v-else>
									<tbody v-if="documents.length!=0">
										<tr role="row" v-for="(document, index) in documents" :key="document.index">
											...
											<td v-if="document.status <=5"><RouterLink :to="{name:'ViewCommentdeptHead',params:{id:document.id}}"> 
												<span :class="document.status == 4 ? 'badge bg-danger' : 'badge bg-success'">{{
												document.status == 4 ? 'Pending' :document.status == 5 ? 'Commented':'Approved' }}</span>
											   </RouterLink>
											</td>					
											<td v-else>
														...
														<ul class="dropdown-menu" aria-labelledby="btnGroupDrop1" style="">
															...
															<li v-if="document.status<=6" ><button  @click.prevent="approveDocument(document.id)"
																	class="btn btn-outline-danger dropdown-item"><i
																		:class="document.status == 5 ? 'fadeIn animated bx bx-plus' : 'fadeIn animated bx bx-minus'"></i>{{
																			document.status == 5 ? 'Approve' : 'Reject' }}</button>
															</li>
