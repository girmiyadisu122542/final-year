...
function addData() {
	...
	formdata.append("advisor",advisor.value);
...
function editData(){
	...
	axios.post('api/update-document/'+userId.value,formdata, {
		headers: {
               'Content-Type': 'multipart/form-data',
             },
		}).then(() => {
		...
		<div v-if="isHide" class="card border-top border-0 border-4 border-primary">
			<div class="card-body p-5">
				...
				<form class="row g-3 " enctype="multipart/form-data">
					...
					<div class="col-12" >
						<label for="validationCustom03" class="form-label">Advisor</label>
						<select v-model="advisor" id="inputState "
							class="form-select  border-primary text-primary">
							<i class="lni lni-funnel"></i>
							<option selected="" disabled="" value="">select advisor</option>
							<option v-for="advisor in advisors" :value="advisor.id">{{ advisor.full_name }}</option>

						</select>
						<div class="text-danger" v-if="errors.advisor && errTime == true">* {{ errors.advisor[0] }}</div>
					...
					<div class="col-md-6" >
						...
						<select v-model="accadamic_year" id="inputState "
							...
							<option v-for="year in back.documents" :value="year.id">{{ year.name }}</option>
