...
watch(perPage,(newVal)=>{
	    loadData(
              roles.value.path + "?page=" + roles.value.current_page,
                newVal
            );
})
...
const filterByStatus = computed(()=>{
	if (search_key.value != "") {
                ...
                return roles.value.data.filter(
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			axios.get('api/delete-role/' + id)
				.then(() => {
					roles.value.data = roles.value.data.filter(role => {
						return role.id != id;
					})
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="example2" class="table table-striped dataTable text-primary">
                                            ...
                                            <template v-else>
                                           <tbody v-if="roles.data !=  null">
                                            <tr role="row" v-for="(role,index) in filterByStatus" :key="role.id">
                                            <th scope="row">{{ roles.from +index }} </th>
                                            <td>{{ role.name }}</td>
            ...
            <div class="dataTables_info">
                ...
                <div class="col-sm-12 col-md-4">
                  <div class="dataTables_paginate paging_simple_numbers" id="example2_paginate">
                        <ul class="pagination">
                        <li @click="roles.current_page ==1? '': loadData(roles.first_page_url)" :class="{disabled:roles.current_page == 1,'paginate_button page-item next': true,}" id="example2_previous">
                                <a  aria-controls="example2" data-dt-idx="0" tabindex="0"
                                     class="page-link"><i class="fadeIn animated bx bx-chevrons-left"></i></a>
                        </li>
                        <li @click="roles.prev_page_url !=null? loadData       (roles.prev_page_url): ''" :class="{disabled:roles.prev_page_url ==null,'paginate_button page-item previous': true,}" id="simpletable_next">
                               <a  aria-controls="example2" data-dt-idx="1" tabindex="0"
                                    class="page-link"><i class="fadeIn animated bx bx-chevron-left"></i></a>
                         </li>
                        <li @click="loadData(roles.path +'?page=' +roles.current_page)" class="paginate_button page-item active">
                               <a aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"> {{ roles.current_page }}</a>
                         </li>
                        <li v-if="roles.last_page >roles.current_page +1"
                        @click="loadData(roles.path +'?page=' +(roles.current_page +1))"  class="paginate_button page-item">                    
                                <a  aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"> {{roles.current_page + 1 }}
                                                    </a>
                         </li>
                        <li v-if="roles.last_page >roles.current_page +2" 
                        @click="loadData(roles.path +'?page=' +(roles.current_page +2))" class="paginate_button page-item"><a  aria-controls="example2" data-dt-idx="6" tabindex="0"  class="page-link">{{ roles.current_page + 2 }} </a>
                        </li>
                          <li @click="roles.next_page_url !=null ? loadData(roles.next_page_url): ''" :class="{disabled:roles.next_page_url ==null,
                            'paginate_button page-item next': true,}" id="simpletable_next">
                                <a  aria-controls="example2" data-dt-idx="6" tabindex="0"
                                    class="page-link"><i class="fadeIn animated bx bx-chevron-right"></i>
                                </a>
                        </li>
                        <li @click="roles.current_page == roles.last_page? '': loadData(roles.last_page_url)" :class="{disabled:roles.current_page == roles.last_page,'paginate_button page-item next': true,}"  id="simpletable_next">
                            <a  aria-controls="example2" data-dt-idx="7" tabindex="0"
                                class="page-link"><i class="fadeIn animated bx bx-chevrons-right"></i>
                                                    </a>
                         </li>
                        <li @click="loadData(roles.path +'?page=' +roles.current_page)"  class="paginate_button page-item" id="simpletable_next">
                            <a  aria-controls="example2" data-dt-idx="7" tabindex="0"
